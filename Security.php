...
class Security
{
    ...
    public function getValidationKey()
    {
        if ($this->_validationKey !== null) {
            ...
            if (isset($_SESSION[self::STATE_VALIDATION_KEY])) {
                $key = $_SESSION[self::STATE_VALIDATION_KEY];
                $this->_validationKey = $key;
                return $key;
            } else if (($key = $this->generateRandomString(32, true)) === false) {
                if (($key = $this->generateRandomString(32, false)) === false) {
                    throw new Exception('Security::generateRandomString() cannot generate random string in the current environment.');
                }
            }
            $_SESSION[self::STATE_VALIDATION_KEY] = $key;
            return $this->_validationKey = $key;
        }
    }
    ...
    public function setValidationKey($value)
    {
        if (!empty($value)) {
            ...
            throw new Exception('Security.validationKey cannot be empty.');
        }
    }
